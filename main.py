...
#              Funciones            
#\***********************************/
def covid_1(PAIS, save_name=None):                                                          #Grafico los nuevos casos y muertes de 1 pais
    ...
    plt.tight_layout()                                                                      #Ajusta el grafico automaticamente
    if (save_name!=None):                                                                   #De haber iingresado un nombre para guardar el grafico
        plt.savefig(str(save_name))                                                         #   ->Se guarda el grafico con el nombre deseado
    return plt.show()                                                                       #Por ultimo, muestro el grafico en el retorno de la función

def covid_compare_2(PAIS1, PAIS2, save_name=None):                                          #Grafico los nuevos casos y muertes de 2 paises, y sus intersecciones
...
def covid_compare_x(l_paises, lapso=None,  save_name=None):                                 #Grafico los nuevos casos de varios X paises
    plt.figure(figsize=(12,4))                                                              #Configuro mi grafico de manera rectangular, haciendolo 3 veces mas ancho que alto.
    plt.title("Nuevos contagios")                                                           #Asigno titulo al grafico
    plt.ylabel('Contagios (miles)')                                                         #Asigno titulo de valores eje Y
    plt.xlabel('Fechas')                                                                    #Asigno titulo de valores eje X
...
def main():                 #Defino mi función main
    print("\nHerramienta: Graficador de casos del COVID-19.\nParticipantes del codigo: ") #Agregar nombre antes de subir
...
opciones = ["1","2","3"]                                                                                                                                                    #Defino mis opciones definidas
...
while True:                                                                                                                                                                 #Creo mi bucle infinito
    ...
    if opcion in opciones:                                                                                                                                                  #Si la opción está en mis opciones:
        if   opcion == "1":                                                                                                                                                 #->Si la opción es la 1
            ...
            if try_pais in paises:                                                                                                                                          #   ->Si el pais está en mi lista de paises
                gh_name = input("Deseas guardar el grafico? Ingresesa el nombre para guardarlo o déjalo en blanco para omitir.\n")                                          #       ->Pregunto si quiere guardar el grafico
                if not bool(gh_name):                                                                                                                                       #           ->De no haber ingresado nombre para el grafico
                    covid_1(try_pais)                                                                                                                                       #               ->Realizo el grafico
                    print(f"Se realizó el grafico de {try_pais}, y se omitió guardarlo.")                                                                                   #               ->Doy un mensaje de exito y explicación de lo que pasó
                else:                                                                                                                                                       #           ->Si dieron un nombre para el grafico
                    covid_1(try_pais,gh_name)                                                                                                                               #               ->Realizo el grafico y lo guardo
                    print(f"Se realizó el grafico de {try_pais}, y se guardo como {gh_name}.")                                                                              #               ->Doy un mensaje de exito y explicación de lo que pasó
            ...
            while not try_pais1 in paises:                                                                                                                                  #   ->Si el primer pais no está en mi lista de paises
                try_pais1 = input(f"No se cuentra {try_pais1} en la lista de paises, pruebe ingresando otra vez el pais 1:\n")                                              #       ->Solicito un nuevo valor, y lo repito hasta que sea válido
            while not try_pais2 in paises:                                                                                                                                  #   ->Idem pais 1
                try_pais2 = input(f"No se cuentra {try_pais2} en la lista de paises, pruebe ingresando otra vez el pais 2:\n")
            gh_name = input("Deseas guardar el grafico? Ingresesa el nombre para guardarlo o déjalo en blanco para omitir.\n")                                              #   ->Idem opción 1 para guardar el grafico, y ver cómo continua
            if not bool(gh_name):
                covid_compare_2(try_pais1, try_pais2)
                print(f"Se realizó el grafico de {try_pais1} y {try_pais2}. Se omitió guardarlo.")
            else:
                covid_compare_2(try_pais1, try_pais2, gh_name)
                print(f"Se realizó el grafico de {try_pais1} y {try_pais2}. Se guardo como {gh_name}.")
            print("\nCómo quieres continuar? Puedes elegir otra opción o salirasdasd con el comando >>  z  <<.")
            opcion = input("Opción deseada:\n")
            ...
            while accion == 1:                                                                                                                                              #->Mientras la accion es igual a 1
                ...
                elif (try_pais == "0"):                                                                                                                                     #   -Sino y si se introdujo un 0
                    print("Necesitas almenos 2 paises para realizar este grafico de comparación.")                                                                          #       ->Doy por hecho que es un error porque no se puede comprar 1 pais con nada
                if try_pais in paises:                                                                                                                                      #   ->Si se introdujo cualquier otra cosa, pruebo si está en mi lista de paises
                    ...
                    if try_pais != "0":                                                                                                                                     #       ->Y si tampoco es un 0
                        print("No se cuentra el pais ingresado")                                                                                                            #       ->Doy un advertencia que el input no fue tomado en cuenta.
            ...
            print("\nCómo quieres continuar? Puedes elegir otra opción o salirasdasd con el comando >>  z  <<.")
            opcion = input("Opción deseada:\n")
    elif opcion == "ayuda":                                                                                                                                                 #Sino y si la opción es "Ayuda"
    ...
    elif opcion == "z":                                                                                                                                                     #Sino y si la opción es "Salir"
...
"""
Grafico a entregar
covid_compare_x(["Argentina", "Chile", "Bolivia", "Paraguay", "Brazil"],["2020-06-21", "2020-09-21"],"Entrega obligatoria")
"""